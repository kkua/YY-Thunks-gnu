...
#if (YY_Thunks_Target < __WindowsNT6_2) && !defined(__Comment_Lib_advapi32)
        ...
        do
        {
            ...
            for (unsigned i = 0; RegEnumKeyW(hTimeZoneRootKey, i, szTimeZoneKeyName, _countof(szTimeZoneKeyName)) == ERROR_SUCCESS;++i)
            {
                ...
                if (lStatus == ERROR_SUCCESS)
                {
                    ...
                    if (StringCompareIgnoreCaseByAscii(StandardName, pTimeZoneInformation->StandardName, _countof(pTimeZoneInformation->StandardName)) == 0)
                    {
