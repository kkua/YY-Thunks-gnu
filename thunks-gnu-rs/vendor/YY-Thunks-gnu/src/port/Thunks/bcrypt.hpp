...
namespace YY::Thunks
{
    ...
    namespace
    {
        ...
        static bool __fastcall TryGetHashAlgInfo(
            _In_opt_z_ LPCWSTR _szHashAlgId,
            _Out_ ALG_ID* _puAlgId,
            _Out_opt_ ULONG* _pcbHash) noexcept
        {
            if (_szHashAlgId == nullptr || _puAlgId == nullptr)
            {
                return false;
            }

            if (StringCompareIgnoreCaseByAscii(_szHashAlgId, BCRYPT_MD2_ALGORITHM, -1) == 0)
            {
                *_puAlgId = CALG_MD2;
                if (_pcbHash)
                {
            ...
            if (StringCompareIgnoreCaseByAscii(_szHashAlgId, BCRYPT_MD4_ALGORITHM, -1) == 0)
            {
                *_puAlgId = CALG_MD4;
                if (_pcbHash)
                {
            ...
            if (StringCompareIgnoreCaseByAscii(_szHashAlgId, BCRYPT_MD5_ALGORITHM, -1) == 0)
            {
                *_puAlgId = CALG_MD5;
                if (_pcbHash)
                {
            ...
            if (StringCompareIgnoreCaseByAscii(_szHashAlgId, BCRYPT_SHA1_ALGORITHM, -1) == 0)
            {
                *_puAlgId = CALG_SHA1;
                if (_pcbHash)
                {
            ...
            if (StringCompareIgnoreCaseByAscii(_szHashAlgId, BCRYPT_SHA256_ALGORITHM, -1) == 0)
            {
                *_puAlgId = CALG_SHA_256;
                if (_pcbHash)
                {
                    *_pcbHash = 32;
                }
            ...
            if (StringCompareIgnoreCaseByAscii(_szHashAlgId, BCRYPT_SHA384_ALGORITHM, -1) == 0)
            {
                *_puAlgId = CALG_SHA_384;
                if (_pcbHash)
                {
                    *_pcbHash = 48;
                }
            ...
            if (StringCompareIgnoreCaseByAscii(_szHashAlgId, BCRYPT_SHA512_ALGORITHM, -1) == 0)
            {
                *_puAlgId = CALG_SHA_512;
                if (_pcbHash)
                {
                    *_pcbHash = 64;
                }
        ...
        struct BCryptAlgorithm : public BCryptObject
        {
            ...
            virtual bool IsPairAlgorithm() const
            {
                return false;
            }
        ...
        struct BCryptAlgorithmByCryptoAPI : BCryptAlgorithm
        {
            static NTSTATUS __fastcall Create(_In_ const BCryptMapItem* _pMapItem, _In_ ULONG _fOpenAlgorithmFlags, _Outptr_ BCryptAlgorithm** _ppAlgorithm)
            {
                *_ppAlgorithm = nullptr;
                ...
                do
                {
                    if (_pMapItem->uProvType == PROV_RSA_FULL || _pMapItem->uProvType == PROV_RSA_AES)
                    {
                        // Windows XP SP3才支持 MS_ENH_RSA_AES_PROV_XP_W
                        ...
                        if (CryptAcquireContextW(&_hProv, nullptr, _szEnhRsaAesProvider, PROV_RSA_AES, CRYPT_VERIFYCONTEXT))
                        {
                            break;
                        }

                        if (CryptAcquireContextW(&_hProv, nullptr, _szEnhRsaAesProvider, PROV_RSA_AES, CRYPT_NEWKEYSET))
                        {
                            break;
                        }
                    }
                    ...
                    if (CryptAcquireContextW(&_hProv, nullptr, _pMapItem->szProvider, _pMapItem->uProvType, CRYPT_NEWKEYSET))
                        break;
        ...
        struct BCryptKey : public BCryptObject
        {
            ...
            virtual bool __fastcall IsKeyPair() const
            {
                return false;
            }
            ...
            virtual NTSTATUS WINAPI Encrypt(
            ...
            virtual NTSTATUS WINAPI Decrypt(
                _In_reads_bytes_opt_(_cbInput)              PUCHAR _pInput,
                _In_                                        ULONG _cbInput,
                _In_opt_                                    VOID* _pPaddingInfo,
                _Inout_updates_bytes_opt_(_cbIV)            PUCHAR _pIV,
        ...
        struct BCryptKeyPair : public BCryptKey
        {
            ...
            bool __fastcall IsKeyPair() const override
            {
                return true;
            }
            ...
            {
                BOOL _bFinal = FALSE;
                if (_fFlags == BCRYPT_PAD_NONE)
                {

                }
                else if (_fFlags == BCRYPT_PAD_PKCS1)
                {
                    _bFinal = TRUE;
                }
                else
                ...
                _In_                                        ULONG _fFlags) override
            {
                ...
                if (!CryptDecrypt(hKey, NULL, _bFinal, 0, _pOutput, &_cbInput))
                {
                    ...
                    return BCryptAlgorithm::CryptoErrorToNTStatus(_uLastError);
                }
                ...
                )
            {
                ...
                DWORD _fCryptoFlags = 0;
                ALG_ID _uHashAlgId = 0;
                ULONG _cbExpectedHash = 0;
                if (dwFlags == BCRYPT_PAD_PKCS1)
                {
                    auto _pPkcs1Info = reinterpret_cast<const BCRYPT_PKCS1_PADDING_INFO*>(pPaddingInfo);
                    if (_pPkcs1Info == nullptr || _pPkcs1Info->pszAlgId == nullptr)
                    {
                        return STATUS_INVALID_PARAMETER;
                    }

                    if (!TryGetHashAlgInfo(_pPkcs1Info->pszAlgId, &_uHashAlgId, &_cbExpectedHash))
                    {
                        return STATUS_NOT_SUPPORTED;
                    }

                    if (cbInput != _cbExpectedHash)
                    {
                        return STATUS_INVALID_PARAMETER;
                    }

                    HCRYPTHASH _hHash = NULL;
                    if (!CryptCreateHash(pAlgorithm->hProv, _uHashAlgId, NULL, 0, &_hHash))
                    {
                        return BCryptAlgorithm::CryptoErrorToNTStatus(GetLastError());
                    }

                    if (!CryptSetHashParam(_hHash, HP_HASHVAL, pbInput, 0))
                    {
                        const auto _uLastError = GetLastError();
                    ...
                    DWORD _uKeySpec = AT_KEYEXCHANGE;
                    if (pAlgorithm->pMapItem->uAlgId == CALG_RSA_SIGN)
                    {
                        _uKeySpec = AT_SIGNATURE;
                    }

                    if (!CryptSignHashW(_hHash, _uKeySpec, nullptr, _fCryptoFlags, nullptr, &_cbSignature))
                    {
                        const auto _uLastError = GetLastError();
                        CryptDestroyHash(_hHash);
                        return BCryptAlgorithm::CryptoErrorToNTStatus(_uLastError);
                    ...
                    {
                        CryptDestroyHash(_hHash);
                        *pcbResult = _cbSignature;
                        return STATUS_SUCCESS;
                    }

                    if (cbOutput < _cbSignature)
                    {
                        CryptDestroyHash(_hHash);
                        *pcbResult = _cbSignature;
                        return STATUS_BUFFER_TOO_SMALL;
                    ...
                    if (!CryptSignHashW(_hHash, _uKeySpec, nullptr, _fCryptoFlags, pbOutput, &_cbSignature))
                    {
                        const auto _uLastError = GetLastError();
                        CryptDestroyHash(_hHash);
                        return BCryptAlgorithm::CryptoErrorToNTStatus(_uLastError);
                    ...
                    MoveAndReverse(pbOutput, pbOutput, _cbSignature);
                    *pcbResult = _cbSignature;
                    return STATUS_SUCCESS;
                }
            ...
            {
                if ((pbHash == nullptr && cbHash != 0) || (pbSignature == nullptr && cbSignature != 0))
                {
                    return STATUS_INVALID_PARAMETER;
                }
                ...
                if (dwFlags == BCRYPT_PAD_PKCS1)
                {
                    ...
                    if (!CryptSetHashParam(_hHash, HP_HASHVAL, pbHash, 0))
                    {
                        const auto _uLastError = GetLastError();
                        CryptDestroyHash(_hHash);
                        return BCryptAlgorithm::CryptoErrorToNTStatus(_uLastError);
                    ...
                    auto _pSignature = reinterpret_cast<BYTE*>(_malloca(cbSignature));
                    ...
                    MoveAndReverse(_pSignature, pbSignature, cbSignature);
                    const auto _bVerifyResult = CryptVerifySignatureW(_hHash, _pSignature, cbSignature, hKey, nullptr, _fCryptoFlags);
                    const auto _uLastError = _bVerifyResult ? ERROR_SUCCESS : GetLastError();
                    _freea(_pSignature);
                    CryptDestroyHash(_hHash);
                    ...
                    if (_uLastError == NTE_BAD_SIGNATURE)
                    {
        ...
        // 密钥对算法的基类
        struct BCryptKeyPairAlgorithm : public BCryptKeyAlgorithm<BCryptKeyPairAlgorithm, BCryptKeyPair, 0, 0>
        {
            bool IsPairAlgorithm() const override
            {
                return true;
            }
            ...
            NTSTATUS WINAPI ImportKeyPair(
                _In_opt_                        BCryptKey* _pImportKey,
                _In_z_                          LPCWSTR _szBlobType,
                _Out_                           BCryptKeyPair** _ppKey,
                _In_reads_bytes_(_cbInput)      PUCHAR  _pInput,
                _In_                            ULONG   _cbInput,
                _In_                            ULONG   _fFlags
                )
            {
                auto _pKeyPair = (BCryptKeyPair*)internal::Alloc(sizeof(BCryptKeyPair));
                if (_pKeyPair == nullptr)
                {
                    return STATUS_NO_MEMORY;
                }

                new(_pKeyPair) BCryptKeyPair(this);

                const HCRYPTKEY _hNativeImportKey = _pImportKey ? _pImportKey->hKey : NULL;
                auto _Status = STATUS_NOT_SUPPORTED;
                ...
                do
                {
                    ...
                    {
                        if (!CryptImportKey(hProv, _pInput, _cbInput, _hNativeImportKey, CRYPT_EXPORTABLE, &_pKeyPair->hKey))
                        {
                            _Status = BCryptAlgorithm::CryptoErrorToNTStatus(GetLastError());
                            break;
                        }
                    ...
                    if (StringCompareIgnoreCaseByAscii(BCRYPT_RSAPUBLIC_BLOB, _szBlobType, -1) == 0
                        || StringCompareIgnoreCaseByAscii(BCRYPT_PUBLIC_KEY_BLOB, _szBlobType, -1) == 0)
                    {
                        if (_cbInput < sizeof(BCRYPT_RSAKEY_BLOB))
                        {
                            _Status = STATUS_INVALID_PARAMETER;
                            break;
                        }

                        auto _pBCryptRsaBlob = reinterpret_cast<const BCRYPT_RSAKEY_BLOB*>(_pInput);
                        ...
                        const BYTE* _pPublicExp = _pInput + sizeof(BCRYPT_RSAKEY_BLOB);
                        DWORD _uPublicExp = 0;
                        ...
                        _pLegacyBlob->aiKeyAlg = pMapItem->uAlgId;
                        ...
                        if (!CryptImportKey(hProv, reinterpret_cast<const BYTE*>(_pLegacyBlob), _cbLegacyBlob, _hNativeImportKey, CRYPT_EXPORTABLE, &_pKeyPair->hKey))
                        {
                            _Status = BCryptAlgorithm::CryptoErrorToNTStatus(GetLastError());
                            _freea(_pLegacyBlob);
                            break;
        ...
        template<>
        bool __fastcall Is<BCryptKeyPair>(void* _pSrc)
        {
            if (!Is<BCryptKey>(_pSrc))
                return false;

            auto _pKey = reinterpret_cast<BCryptKey*>(_pSrc);
            return _pKey->IsKeyPair();
        }

        template<>
        bool __fastcall Is<BCryptKeyPairAlgorithm>(void* _pSrc)
        {
            if (!Is<BCryptAlgorithm>(_pSrc))
                return false;

            auto _pAlgorithm = reinterpret_cast<BCryptAlgorithm*>(_pSrc);
            return _pAlgorithm->IsPairAlgorithm();
        }
    }
#endif
}
...
namespace YY::Thunks
{
        ...
        )
    {
        ...
        {
            { BCRYPT_RSA_ALGORITHM, nullptr, PROV_RSA_FULL, CALG_RSA_KEYX, &BCryptKeyPairAlgorithm::Create },
            { BCRYPT_RSA_SIGN_ALGORITHM, nullptr, PROV_RSA_FULL, CALG_RSA_SIGN, &BCryptKeyPairAlgorithm::Create },
            // BCRYPT_DH_ALGORITHM
        ...
        )
    {
        ...
        if (!Is<BCryptKeyPairAlgorithm>(_hAlgorithm))
            return STATUS_NOT_SUPPORTED;

        return reinterpret_cast<BCryptKeyPairAlgorithm*>(_hAlgorithm)->GenerateKeyPair(reinterpret_cast<BCryptKey**>(_phKey), _uBitLength, _fFlags);
    }
#endif
        ...
        )
    {
        ...
        auto _pKey = reinterpret_cast<BCryptKey*>(_hKey);
        if (!Is<BCryptKeyPairAlgorithm>(_pKey->pAlgorithm))
        {
            return STATUS_NOT_SUPPORTED;
        }
        ...
        )
    {
        ...
        if (_phKey == nullptr || _szBlobType == nullptr || (_pInput == nullptr && _cbInput != 0) || _fFlags)
        {
            return STATUS_INVALID_PARAMETER;
        }
        ...
        if (_hImportKey && !Is<BCryptKey>(_hImportKey))
        {
            return STATUS_INVALID_HANDLE;
        }
        ...
        *_phKey = nullptr;
        auto _pKeyPairAlgorithm = reinterpret_cast<BCryptKeyPairAlgorithm*>(_hAlgorithm);
        return _pKeyPairAlgorithm->ImportKeyPair(reinterpret_cast<BCryptKey*>(_hImportKey), _szBlobType, reinterpret_cast<BCryptKeyPair**>(_phKey), _pInput, _cbInput, _fFlags);
    }
#endif
    ...
    {
        if (const auto _pfnBCryptSignHash = try_get_BCryptSignHash())
        {
            return _pfnBCryptSignHash(hKey, pPaddingInfo, pbInput, cbInput, pbOutput, cbOutput, pcbResult, dwFlags);
        }

        if (pcbResult == nullptr || (pbInput == nullptr && cbInput != 0))
        {
            return STATUS_INVALID_PARAMETER;
        }

        if (!Is<BCryptKey>(hKey))
        {
            return STATUS_INVALID_HANDLE;
        }
        
        if (!Is<BCryptKeyPair>(hKey))
        {
            return STATUS_NOT_SUPPORTED;
        }

        auto _pKey = reinterpret_cast<BCryptKeyPair*>(hKey);
        return _pKey->SignHash(pPaddingInfo, pbInput, cbInput, pbOutput, cbOutput, pcbResult, dwFlags);
    }
#endif
    ...
    {
        if (const auto _pfnBCryptVerifySignature = try_get_BCryptVerifySignature())
        {
            return _pfnBCryptVerifySignature(hKey, pPaddingInfo, pbHash, cbHash, pbSignature, cbSignature, dwFlags);
        }
