...
namespace YY::Thunks
{
    ...
    ReclaimVirtualMemory,
        _In_reads_(_uSize) PVOID _pVirtualAddress,
        _In_ SIZE_T _uSize
        )
    {
...
#if defined(__ENABLE_WORKAROUND_2_UNNAME_OBJECT_DACL) && (YY_Thunks_Target < __WindowsNT6_3)
    ...
    CreateFileMappingW,
        _In_ HANDLE _hFile,
        _In_opt_ LPSECURITY_ATTRIBUTES _pFileMappingAttributes,
        _In_ DWORD _flProtect,
        _In_ DWORD _uMaximumSizeHigh,
        _In_ DWORD _uMaximumSizeLow,
        _In_opt_ LPCWSTR _szName
        )
    {
        const auto _pfnCreateFileMappingW = try_get_CreateFileMappingW();
        if (!_pfnCreateFileMappingW)
        {
        ...
        wchar_t _szTmpMapName[256];
        if (_pFileMappingAttributes && (_szName == nullptr || *_szName == L'\0') && internal::GetSystemVersion() < internal::MakeVersion(6, 3))
        {
            internal::StringBuffer<wchar_t> _TmpBuild(_szTmpMapName, _countof(_szTmpMapName));
            _TmpBuild.AppendString(L"YY_Thunks_Unnamed_Section_");
            GUID _Id;
            const auto _pfnRtlGenRandom = try_get_SystemFunction036();
            _pfnRtlGenRandom(&_Id, sizeof(_Id));
            _TmpBuild.AppendGUID(_Id);

            _szName = _szTmpMapName;
        }
#endif

        return _pfnCreateFileMappingW(_hFile, _pFileMappingAttributes, _flProtect, _uMaximumSizeHigh, _uMaximumSizeLow, _szName);
    }
#endif
}
